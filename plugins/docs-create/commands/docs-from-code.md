...
# /docs-from-code — extract a code repo into Markdown docs
...
Next:
  ...
  /docs-create               — run all stages in one command
```

## Failure handling

- `{"status":"error","reason":"clone_failed"}` → print `hint` and `detail`. Suggest `gh auth login` for private repos.
- `{"status":"error","reason":"no_readme"}` → tell the user the repo has no usable README and suggest passing a different source or running `/docs-from-site <url>` if a marketing site exists.
