...
End-to-end orchestrator. Detects the source type, picks one of three pinned builder subagents, then chains publish + configure:
...
1. Run `tree <path> -L 2 --noreport` (or `find <path> -maxdepth 2 -type f | sort` if `tree` is missing). Print the tree verbatim.
2. Pick up to 3 representative pages — `README.md` first, then the largest two `.md` files. For each, print the first 15 lines as a fenced block prefixed with the relative path.
3. Print the one-line summary (`<pages> pages, branding: <accent> <scheme>, favicon: <yes/no>`).
...
```
✅ Docs ready at <path> (<pages> pages).
