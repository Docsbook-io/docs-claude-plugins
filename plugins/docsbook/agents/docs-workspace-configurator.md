...
model: sonnet
tools: Read, mcp__plugin_docsbook_docsbook__list_workspaces, mcp__plugin_docsbook_docsbook__get_workspace, mcp__plugin_docsbook_docsbook__create_workspace, mcp__plugin_docsbook_docsbook__update_branding, mcp__plugin_docsbook_docsbook__update_ui_settings, mcp__plugin_docsbook_docsbook__update_navigation, mcp__plugin_docsbook_docsbook__update_seo, mcp__plugin_docsbook_docsbook__update_geo, mcp__plugin_docsbook_docsbook__update_aeo, mcp__plugin_docsbook_docsbook__update_languages, mcp__plugin_docsbook_docsbook__update_ai_settings, mcp__plugin_docsbook_docsbook__set_chat_system_prompt, mcp__plugin_docsbook_docsbook__update_domain
---
...
The orchestrator (`/docsbook:run-docs-create`) generates concrete values (system prompt, custom questions, subheader items, SEO meta) and passes them in. **Do not invent these values yourself** — if a field is missing from the input, skip that step and log a warning. The user already confirmed exactly what to apply.
...
1. **MCP probe.** Call `mcp__plugin_docsbook_docsbook__list_workspaces`. If it fails for transport reasons (not auth), return `{"status":"mcp_unavailable","instructions":["mcp add --transport http https://docsbook.io/api/mcp/server","then re-run /docsbook:run-docs-setup-workspace"]}` and exit.

2. **Resolve workspace.** Look for an existing workspace matching `owner/repo`. If absent, call `mcp__plugin_docsbook_docsbook__create_workspace({github_owner: owner, github_repo: repo})`. Store the workspace id.
...
5. **Apply settings in this order — each in a try/catch:**
   ...
   - **`update_navigation`**: combine two things into one call:
     ...
     - `subheader` (or whatever the MCP tool calls the secondary nav — inspect `mcp__plugin_docsbook_docsbook__update_navigation`'s schema): the `subheaderFolders` array from input, verbatim. If `subheaderFolders` is absent or empty, only push `headerLinks`.
