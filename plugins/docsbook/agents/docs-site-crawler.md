...
|---|---|---|
| 403 / 401 on homepage | `auth_required` | "Site requires login — try `/docsbook:run-docs-from-code <repo>` if source is on GitHub." |
| Homepage HTML has <500 chars or no `<main>`/`<article>` | `spa_no_ssr` | "Looks like a client-rendered SPA. Try `/docsbook:run-docs-from-code <repo>` instead, or fetch with a headless browser." |
| Strict CSP / Cloudflare challenge page detected | `bot_blocked` | "Bot protection blocked the crawl. Use `/docsbook:run-docs-from-code <repo>` if source is on GitHub." |
...
```
{"status":"error","reason":"spa_no_ssr","path":"<outputPath echoed verbatim>","hint":"Looks like a client-rendered SPA. Try /docsbook:run-docs-from-code <repo> instead.","detail":"homepage HTML returned 312 chars, no <main>"}
```
