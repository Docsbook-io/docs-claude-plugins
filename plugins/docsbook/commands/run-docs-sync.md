...
# /docsbook:run-docs-sync — code↔docs drift orchestrator
...
```text
INTENT = "$ARGUMENTS"           # raw text passed after /docsbook:run-docs-sync
...
Before doing anything, remove leftover worktrees from previous `/docsbook:run-docs-sync` runs. They are safe to delete — each run creates fresh ones.
...
> **Intent mode skips Step 0b.** The push-gate marker is only relevant when docs-sync runs because of a git push. When the user explicitly invokes `/docsbook:run-docs-sync <intent>`, do not touch the marker file.
...
> It runs `claude --print /docsbook:run-docs-sync` in a separate headless session and is **not**
...
if git diff --cached --quiet; then
  ...
  if [ "$MODE" = "intent" ]; then
    ...
    Generated by /docsbook:run-docs-sync intent mode.
    Intent: $INTENT"
  else
...
if git diff --cached --quiet; then
  ...
  if [ "$MODE" = "intent" ]; then
    ...
    Generated by /docsbook:run-docs-sync. See main repo commit for code diff."
  fi
...
First print one line, on its own, before the JSON:

```text
DOCS_SYNC_DRIFT: <count of files_changed>
```

This is what the pre-push git hook reads to decide whether `DOCS_SYNC_MODE=block` should stop the push. It is a plain line rather than a JSON field because the hook is a shell script that cannot assume `jq` exists — and because block mode was inert for exactly as long as the hook had nothing but an exit code to go on: a clean run that found ten drifted pages exited 0 and sailed straight through. Print `DOCS_SYNC_DRIFT: 0` when there is no drift; never omit the line.

Then print a single JSON object to stdout — nothing else after this. The format is part of the public contract so parent orchestrators (and CI) can rely on it:
