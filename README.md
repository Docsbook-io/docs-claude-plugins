...
**One install: 52 documentation skills, 40 commands, 19 pinned subagents, and the Docsbook MCP server.**
...
| | Count | Namespaced as |
|---|---|---|
| **Skills** — the documentation knowledge: audits, authoring rules, SEO, i18n, analytics playbooks | 52 | `docsbook:<name>` |
...
Read `docsbook:docs-seo` to learn what good SEO looks like; run `/docsbook:run-docs-seo-audit` to have it checked.
...
```
docs-skills          the knowledge — 52 SKILL.md files, works in any agent
  └─ synced into ─→  docsbook plugin   the same 52 + commands + subagents + MCP
docsbook.io/get-started.md             the playbook that installs either one
```

Skills are **not** authored here. They live in [Docsbook-io/docs-skills](https://github.com/Docsbook-io/docs-skills) and are copied in from a version-pinned npm release by `scripts/sync-skills.mjs`, which flattens the two-level source layout, rewrites the relative `metric_dictionary` paths, and fails loudly on a name collision.

CI re-runs that sync and **fails if the committed tree differs by a byte**, so a stale catalog cannot merge. To take a new catalog release, bump `UPSTREAM_VERSION` in the script, run `npm run sync-skills`, and commit — the `.sync-lock.json` diff shows exactly which skills moved.
