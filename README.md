...
3. **`.github/workflows/sync-upstream.yml`, on every upstream push and hourly.** Moves the pin to `docs-skills@main`, re-runs both syncs, and commits. Nothing here depends on a person remembering that upstream released. docs-skills fires a `upstream-push` `repository_dispatch` from its own push-to-`main` job, so a release normally lands here in seconds; the hourly cron is the backstop for the dispatches that never arrive, and the only thing that refreshes the MCP tool list, which drifts on the server's deploy schedule rather than on any push.

The pin is a **git commit, not an npm version**. It used to be `docs-skills@<version>` from npm, which reads as the safer choice and was the reason this plugin went eleven days shipping a catalog that no longer existed: docs-skills' publish workflow bumps the version *before* it publishes, so when its `NPM_TOKEN` lost publish rights the bump commits kept landing on `main` and the registry silently stopped at 1.8.29. A pin can only be as fresh as the registry it reads. (Upstream has since reordered that workflow to publish before it records the release, and to refuse the run outright when the token cannot publish — but the pin stays on git, because this repository should not depend on another one's release step succeeding.)
